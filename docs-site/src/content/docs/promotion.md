...
description: Как настроить визуальную презентацию и SEO для репозитория Yandex Station
---

Этот документ описывает, как поддерживается визуальное представление репозитория **Yandex Station** на GitHub и в поиске.
...
- **Badge "Music Assistant"** — динамический shields.io endpoint badge: `https://trudenboy.github.io/ma-provider-tools/badges/yandex_station.json`. Cron `update-ma-version-badges.yml` обновляет JSON каждые 4 часа и показывает, в какие каналы MA (stable / beta / dev) включена текущая версия провайдера.
