...
async def test_announcement_waits_for_resolved_duration() -> None:
    ...
    player.glagol = SimpleNamespace(send=mock.AsyncMock(return_value={"status": "SUCCESS"}))
    duration_resolver = mock.AsyncMock(return_value=4)
    player.mass = SimpleNamespace(
        streams=SimpleNamespace(get_announcement_duration=duration_resolver)
    )
    announcement = SimpleNamespace(uri="http://ma.local/announcement.mp3", duration=1)
...
async def test_announcement_uses_media_duration_without_resolver() -> None:
    ...
    player._attr_volume_level = 20
    player.glagol = SimpleNamespace(send=mock.AsyncMock(return_value={"status": "SUCCESS"}))
    player.mass = SimpleNamespace(streams=SimpleNamespace())
    announcement = SimpleNamespace(uri="http://ma.local/announcement.mp3", duration=1)
